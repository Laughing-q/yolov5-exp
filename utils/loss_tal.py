...
class ComputeLoss:
    ...
    #         anchor_points = torch.stack([shift_x, shift_y], axis=-1).clone().to(p[0].dtype)  # (h, w, 2)
    #         anchor_points = anchor_points.reshape([-1, 2])  # (h*w, 2)
    #
    #         target_labels, target_bboxes, target_scores, fg_mask = self.assigner(
    #             pcls.detach(), pred_bboxes.detach(), anchor_points, gt_labels, xywh2xyxy(gt_bboxes), mask_gt
    #         )
    #         n = fg_mask.sum()
    #         if n:
